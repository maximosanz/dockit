...
#    url(r'^refine_search_(?P<target>[\S|^\_]+)_(?P<method>[\S|^\_]+)_(?P<refinement>[\S|^\_]+)_(?P<i_rmsd_threshold>[\S|^\_]+)_(?P<l_rmsd_threshold>[\S|^\_]+)_(?P<r_rmsd_threshold>[\S|^\_]+)_(?P<fnat_threshold>[\S|^\_]+)_(?P<rank_str>[\S|^\_]+)_(?P<count>\w+)_(?P<target_selected>[\S|^\_]+)/$', views.refine_search, name='refine_search'),
)
